...
def deepLearningIndex(bounding_boxes, ground_boxes, confidence_threshold, TP, FP, iou_threshold=0.1, S = 7, B = 2):
    # 深度学习指标统计
    ...
    for rowIndex in range(S):
        for colIndex in range(S):
            ...
            #如果此处实际是物体 并且预测到了物体
            if round(ground_box[4]) == 1 and bounding_box[4] >= confidence_threshold:
                #拿到实际物体的类别标签
                ground_object_num = ground_object_num + 1
                bounding_index = np.argmax(bounding_box[5:])
                ground_index = np.argmax(ground_box[10:])
                #预测是物体 且 预测类别正确
                if bounding_index == ground_index:
                    ...
                    if iou(bounding_box[0:4], ground_box[5:9]) >= iou_threshold:
                        ...
                        TP[bounding_index] = TP[bounding_index] + 1
                    #iou值不够大
                    else:
                        FP[bounding_index] = FP[bounding_index] + 1
...
for confidence_cycle in range(5, 95, 30):
    ...
    with tqdm(total=len(all_bounding_boxes)) as tbar:
        ...
        for sample_index in range(len(all_bounding_boxes)):
            ...
            deepLearningIndex(bounding_boxes, ground_boxes, confidence_threshold, TP, FP)
            ...
            for classIndex in range(classNum):
                ...
                recall = TP[classIndex] / dataSet.class_bbox_nums[classIndex]
