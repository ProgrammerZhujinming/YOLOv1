...
3.初步撰写了mAP计算脚本  
4.将BN层调整在ReLU层后面，有实验证明，对于类sigmod型的函数，先BN再激活比较好，会有将激活函数输出拉到梯度不饱和区间的位置的作用，可以加速训练；放置在ReLU后面是因为ReLU的梯度饱和问题比sigmod轻得多，并且BN层接在下一层之前，效果相当于将下一层的输入做归一化，正如我们在训练之前对数据集中的数据进行归一化那样。  